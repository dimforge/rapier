...
//! is a motor-driven arm on a revolute joint (axis Z) whose limit range is drawn from a
//! different family. The top row is driven counter-clockwise and parks at the range's max;
//! the bottom row is driven clockwise and parks at the min. The last column's range is
//! wider than a full turn, which is indistinguishable from "no limit" for a wrapped angle:
//! those arms spin forever.
...
pub async fn run(viewer: &mut TestbedViewer) -> anyhow::Result<()> {
    ...
    for (i, (limits_deg, _)) in LIMITS_DEG.iter().enumerate() {
        // dir = 1: driven counter-clockwise (looking down -Z) toward the max;
        // dir = -1: toward the min.
        for dir in [1.0f32, -1.0] {
            let center = Vector::new(
                i as f32 * spacing,
                if dir > 0.0 { 0.0 } else { -spacing },
                0.0,
            );

            // A small fixed ball marking the dial's center.
            let anchor = world
                .bodies
            ...
            let arm = world.bodies.insert(
                RigidBodyBuilder::dynamic()
                    .translation(center + Vector::new(1.0, 0.0, 0.0))
                    .angular_damping(3.0)
                    .can_sleep(false),
            );
            world.insert_collider(ColliderBuilder::cuboid(0.5, 0.1, 0.1), Some(arm));
    ...
    viewer.look_at(
        Vec3::new(2.0 * spacing, -0.5 * spacing, 25.0),
        Vec3::new(2.0 * spacing, -0.5 * spacing, 0.0),
    );
