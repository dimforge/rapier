//! The concrete 3D / `f32` rigid-body / dynamics `#[pyclass]` types
//! (rigid bodies and builders, mass properties, integration parameters, the
//! island manager, the CCD solver, ...) plus the `register_dynamics` function
//! used by `#[pymodule]`.
//!
...
//!
//! Forward-compat with later phases: the module relies on `ColliderHandle`,
//! `ColliderSet`, `ImpulseJointSet` and `MultibodyJointSet` being in scope
//! (they live in `geometry.rs` / `joints.rs`).
...
#[allow(dead_code)]
#[inline]
fn _na_vec_to_engine(v: crate::na::SVector<Real, 3>) -> rapier::math::Vector {
    v.into()
}
...
#[allow(dead_code)]
#[inline]
fn _engine_vec_to_na(v: rapier::math::Vector) -> crate::na::SVector<Real, 3> {
    v.into()
}
...
impl RigidBodyType {
    #[inline]
    pub(crate) fn to_rapier(self) -> rapier::dynamics::RigidBodyType {
        match self {
    ...
    #[inline]
    pub(crate) fn from_rapier(t: rapier::dynamics::RigidBodyType) -> Self {
        match t {
...
impl CoefficientCombineRule {
    #[allow(dead_code)]
    #[inline]
    pub(crate) fn to_rapier(self) -> rapier::dynamics::CoefficientCombineRule {
        match self {
    ...
    #[allow(dead_code)]
    #[inline]
    pub(crate) fn from_rapier(r: rapier::dynamics::CoefficientCombineRule) -> Self {
        match r {
...
impl IslandManager {
    ...
    fn __iter__(slf: PyRef<'_, Self>) -> PyResult<Py<RigidBodyHandleIter>> {
        let handles: Vec<RigidBodyHandle> = slf.0.active_bodies().map(RigidBodyHandle).collect();
...
impl LockedAxes {
    ...
    #[classattr]
    const ROTATION_LOCKED: LockedAxes = LockedAxes(rapier::dynamics::LockedAxes::ROTATION_LOCKED);
...
impl RigidBody {
    pub(crate) fn new_owned(body: rapier::dynamics::RigidBody) -> Self {
        RigidBody {
...
impl RigidBody {
    ...
    #[staticmethod]
    #[pyo3(signature = (**kwargs))]
    fn fixed(kwargs: Option<&Bound<'_, crate::pyo3::types::PyDict>>) -> PyResult<RigidBodyBuilder> {
...
impl RigidBodyBuilder {
    fn apply_angvel_kwarg(
        ...
        let pv: PyVector = v.extract()?;
        let g: rapier::math::Vector = pv.0.into();
        *builder = builder.clone().angvel(g);
        Ok(())
    }
    ...
    fn apply_rotation_kwarg(
        ...
        let pv: PyVector = v.extract()?;
        let g: rapier::math::Vector = pv.0.into();
        *builder = builder.clone().rotation(g);
        Ok(())
    }
...
impl RigidBodySet {
    ...
    #[new]
    fn new() -> Self {
        Self(rapier::dynamics::RigidBodySet::new())
    }
...
impl RigidBodySetIter {
    ...
    fn __next__(mut slf: PyRefMut<'_, Self>) -> Option<(RigidBodyHandle, RigidBody)> {
        ...
        Some((
            ...
            RigidBody {
                backing: RigidBodyBacking::InSet { set, handle },
            },
        ))
    }
}
...
// ----------------------------------------------------------------------
// register_dynamics — module entry point.
// ----------------------------------------------------------------------
