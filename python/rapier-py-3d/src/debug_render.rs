...
struct DebugLine {
    object_kind: u32,
    a: crate::na::SVector<Real, 3>,
    b: crate::na::SVector<Real, 3>,
    color: [f32; 4],
}
...
impl DebugRenderMode {
    ...
    #[classattr]
    const JOINTS: DebugRenderMode = DebugRenderMode(rapier::pipeline::DebugRenderMode::JOINTS);
    #[classattr]
    const SOLVER_CONTACTS: DebugRenderMode =
        DebugRenderMode(rapier::pipeline::DebugRenderMode::SOLVER_CONTACTS);
    #[classattr]
    const CONTACTS: DebugRenderMode = DebugRenderMode(rapier::pipeline::DebugRenderMode::CONTACTS);
    #[classattr]
...
impl DebugRenderStyle {
    ...
    ///
    /// Accepts a :class:`DebugColor` or a 4-tuple HSLA.
    fn set_multibody_joint_separation_color(&mut self, v: &Bound<'_, PyAny>) -> PyResult<()> {
...
impl DebugLineCollector {
    ...
    fn __iter__(slf: PyRef<'_, Self>) -> PyResult<Py<DebugLineCollectorIter>> {
        ...
        let items: Vec<(DebugRenderObject, Vec3, Vec3, DebugColor)> = lines
            .iter()
            .map(|ln| {
                (
                    DebugRenderObject {
...
pub struct DebugLineCollectorIter {
    items: Vec<(DebugRenderObject, Vec3, Vec3, DebugColor)>,
    i: usize,
}
...
impl DebugLineCollectorIter {
    ...
    ) -> Option<(DebugRenderObject, Vec3, Vec3, DebugColor)> {
...
impl rapier::pipeline::DebugRenderBackend for _CollectorBackendAdapter {
    fn draw_line(
        ...
        let an: crate::na::SVector<Real, 3> = a.into();
        let bn: crate::na::SVector<Real, 3> = b.into();
...
impl DebugRenderPipeline {
    ...
    fn render(
        ...
        let err_slot: _DbgArc<_DbgMutex<Option<PyErr>>> = _DbgArc::new(_DbgMutex::new(None));
    ...
    fn render_to_arrays<'py>(
        ...
        let objects_arr =
            PyArray1::<u32>::from_vec_bound(py, lines.iter().map(|ln| ln.object_kind).collect());
