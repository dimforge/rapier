...
#
# - JavaScript: prepares the non-deterministic 2D/3D packages of `typescript/`, builds them, runs
#   typedoc, and copies the result to static/javascript2d and static/javascript3d.
#   Requires cargo, wasm-pack and npm (see typescript/README.md).
# - C: runs the Doxygen reference of `c/doxygen` (all dimension/precision variants) and copies the
#   HTML to static/c. Requires cmake, a C compiler, python3 and Doxygen 1.9.4+.
# - Python: builds the `rapier3d` package and its Sphinx docs with `python/dev.sh docs` (in the virtual
#   environment it manages, see python/README.md), and copies the HTML to static/python.
...
build_js() {
    ...
    for dim in 2 3; do
        ...
        cp -r "$RAPIER_DIR/typescript/builds/rapier${dim}d/docs" "$STATIC_DIR/javascript${dim}d"
...
build_c() {
    ...
    cmake -S "$RAPIER_DIR/c/doxygen" -B "$build_dir"
...
build_py() {
    ...
    rm -rf "$RAPIER_DIR/python/docs/_build/html"
    "$RAPIER_DIR/python/dev.sh" docs
    rm -rf "$STATIC_DIR/python"
    cp -r "$RAPIER_DIR/python/docs/_build/html" "$STATIC_DIR/python"
