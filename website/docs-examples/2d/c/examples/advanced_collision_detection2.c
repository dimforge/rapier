...
static int same_collider(R2ColliderHandle a, R2ColliderHandle b) {
...
int main(void) {
    ...
    R2ColliderHandle collider_handle1 = r2InsertColliderWithoutParent(world, &ground);
    ...
    ball.activeEvents = R2_COLLISION_EVENTS | R2_CONTACT_FORCE_EVENTS;
    R2ColliderHandle collider_handle2 = r2InsertCollider(ball_body_handle, &ball);

    /* A sensor intersecting the ground (both are fixed). */
    R2ColliderDesc sensor = r2BallColliderDesc(0.5);
    sensor.position.translation = r2Vector(5.0, 0.0);
    sensor.isSensor = 1;
    sensor.activeCollisionTypes = R2_COLLISION_TYPES_DEFAULT | R2_COLLISION_TYPES_FIXED_FIXED;
    r2InsertColliderWithoutParent(world, &sensor);
    ...
    size_t collision_count = r2EventCollector_CollisionEvents(events, NULL, 0);
    size_t force_count = r2EventCollector_ContactForceEvents(events, NULL, 0);
    ...
    R2UserData even = {2, 0};
    r2Collider_SetUserData(collider_handle1, even);
    r2Collider_SetUserData(collider_handle2, even);
    r2Collider_SetActiveHooks(collider_handle2, R2_FILTER_CONTACT_PAIRS | R2_MODIFY_SOLVER_CONTACTS);
    r2RigidBody_WakeUp(ball_body_handle, 1);
    for (int i = 0; i < 10; i++) {
