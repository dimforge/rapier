...
fn main() {
    ...
    let handle1 = world.insert_collider(ColliderBuilder::cuboid(1.0, 1.0).translation(Vector::new(0.0, 10.0)), None);
    let handle_to_remove =
        world.insert_collider(ColliderBuilder::cuboid(1.0, 1.0).translation(Vector::new(0.0, 15.0)), None);
    world.remove_collider(handle_to_remove);
    world.colliders[handle1].set_translation(Vector::new(0.0, 12.0));
