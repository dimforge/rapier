        ...
        } = *ctx;
        {
            ...
            for mesh in sb.meshes().filter(|mesh| mesh.collision_enabled()) {
                ...
                for pair in narrow_phase.contact_pairs_with(surface_handle) {
                    ...
                    // A closed surface's reversed interior contacts (a body that crossed it
                    // The rule reads the cells' boundary and its winding, which a skin is
                    // not: a skinned body collides plainly.
                    let skinned = mesh.is_skinned();
                    let closed = mesh.is_closed() && self_particle.is_none() && !skinned;

                            // The row tracks the barycentric point of the element: its reference
                            // The point the constraint tracks, expressed in the particles it acts through.
                            // ghost of that internal vertex/edge (the neighbor carries the
                            // once, their vertex contacts are real support.
                    }
