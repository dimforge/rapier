/* Port of examples2d/stress_tests/capsules2.rs. */
#include "testbed.h"
#include "rapier_helpers.h"
#include "rapier_math.h"

void tbStressTestsCapsules2(Testbed *testbed) {
    /* World. */
    ...
    for (int side = -1; side <= 1; side += 2) {
        ...
        rigidBody.position.translation = r2Vector(side * 25, 100);
        rigidBody.position = r2Pose(r2Vector(side * 25, 100), r2Rotation(R2_PI / 2));
        R2ColliderDesc collider = r2CuboidColliderDesc(r2Vector(100, 1.2));
    ...
    for (int i = 0; i < 26; i++) {
        for (int j = 0; j < 130; j++) {
            ...
            rigidBody.position.translation = r2Vector(i - 13, j * 2.5 + 3.5);
            R2ColliderDesc collider = r2CapsuleYColliderDesc(0.75, 0.5);
            rigidBody.canSleep = !testbed->noSleep;
            R2RigidBodyHandle rigidBodyHandle = r2InsertRigidBody(world, &rigidBody);
            r2InsertCollider(rigidBodyHandle, &collider);
        }
