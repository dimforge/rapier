...
#[repr(transparent)]
/// A pin-slot joint, locks all relative motion between two bodies except for translations along the joint’s
/// principal axis and relative rotations.
///
/// This joint is also known as a groove joint. It is only available in 2D.
...
impl PinSlotJoint {
    /// Creates a new pin-slot joint allowing only relative rotations and relative translations along the specified axis.
    ///
...
impl From<PinSlotJoint> for GenericJoint {
    fn from(joint: PinSlotJoint) -> GenericJoint {
        joint.data
    }
}
...
///
/// A pin-slot joint locks all relative motion except for translations along the joint’s principal axis
/// and relative rotations.
#[cfg_attr(feature = "serde-serialize", derive(Serialize, Deserialize))]
#[derive(Copy, Clone, Debug, PartialEq)]
pub struct PinSlotJointBuilder(PinSlotJoint);
...
impl From<PinSlotJointBuilder> for TypedJoint {
    fn from(joint: PinSlotJointBuilder) -> TypedJoint {
        joint.0.into()
    }
}

impl From<PinSlotJoint> for TypedJoint {
    fn from(joint: PinSlotJoint) -> TypedJoint {
        TypedJoint::PinSlotJoint(joint)
    }
}
